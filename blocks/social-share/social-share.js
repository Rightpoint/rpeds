...
export default function decorate(block) {
  ...
  // Get platforms from block content or use defaults
  const contentText = block.textContent.trim().toLowerCase();
  const requestedPlatforms = contentText
    ? contentText.split(/[,\s]+/).filter((p) => SOCIAL_PLATFORMS[p])
    : ['facebook', 'twitter', 'linkedin', 'email', 'copy'];

  const wrapper = document.createElement('div');
  wrapper.className = 'social-share-wrapper';

  // Label
  if (!block.classList.contains('no-label')) {
  ...
  buttons.className = 'social-share-buttons';

  requestedPlatforms.forEach((platformKey) => {
    const platform = SOCIAL_PLATFORMS[platformKey];
    if (!platform) return;

    const btn = document.createElement('button');
    btn.className = `social-share-btn social-share-${platformKey}`;
    btn.setAttribute('aria-label', `Share on ${platform.name}`);
    ...
    if (platformKey === 'copy') {
      btn.addEventListener('click', async () => {
        try {
          ...
          btn.classList.add('social-share-copied');
          setTimeout(() => btn.classList.remove('social-share-copied'), 2000);
        } catch (e) {
          // Fallback
      ...
      btn.addEventListener('click', () => {
        const shareUrl = platform.getUrl(pageUrl, pageTitle);
